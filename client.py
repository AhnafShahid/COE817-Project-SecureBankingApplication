...
class ClientApp:
    def __init__(self, master):
        ...
        self.sharedKey = bytearray(b'qwertyuiop')
        self.master = master
    ...
    def show_transaction_ui(self):
        ...
        image_label.pack(pady=(20, 30))

        

        transaction_frame = tk.Frame(self.master, bg=background_color)
        transaction_frame.pack(pady=(0, 20))
        tk.Label(transaction_frame, text="Select Action:", bg=background_color, fg=text_color).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(transaction_frame, text="Check Balance", variable=self.action_var, value="balance", bg=background_color, fg=text_color).pack(side=tk.LEFT)
        ...
        submit_button = tk.Button(self.master, text="Submit", bg="black", fg="white", command=self.perform_transaction)
        submit_button.pack()
    ...
    def connect(self):
        try:
            ...
            bankSock.connect(('localhost', 8000))
            atmNonce = os.urandom(16)
            print("ATM Nonce:", atmNonce)  
            ...
            print("Decrypted Bank Message:", bankMessage)  
            bankNonce = bankMessage.split(b'||')[0]
            receivedNonce = bankMessage.split(b'||')[1]

            print("Received Bank Server Nonce:", bankNonce)  
            print("Received ATM Nonce from Bank:", receivedNonce)  
            if receivedNonce == atmNonce:
                ...
                self.encryption_key, self.mac_key = derive_keys(self.masterKey) 
                print("Encryption Key:", self.encryption_key)  
                print("MAC Key:", self.mac_key)  

                self.mac_key = self.mac_key
            else:
                print("Received Nonce did not match ATM Nonce. Authentication failed.")  
            return bankSock
        except Exception as e:
        ...
        if self.bankSock:
            try:
                if action != 'logout':
                    ...
                    print("Received encrypted response from server:", encrypted_response)  
